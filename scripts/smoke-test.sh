...
#
# Live smoke test — open.gerege.mn (эсвэл BASE=... өөр хост).
...
#
# Хэрэглээ:  BASE=https://open.gerege.mn scripts/smoke-test.sh
# Гаралт: PASS/FAIL мөрүүд; ямар нэг FAIL байвал exit code 1.

set -uo pipefail

BASE="${BASE:-https://open.gerege.mn}"
ORIGIN="${BASE}"
PASS=0
FAIL=0
...
redirect_code=$(curl -s -m 15 -o /dev/null -w '%{http_code}' "http://open.gerege.mn/")
