...
| `ALLOWED_ORIGINS` | `https://open.gerege.mn` | Liste d'origines CORS (séparées par des virgules ; `*` interdit) |
...
| `SSO_REDIRECT_URI` | `https://open.gerege.mn/sso/callback` | Doit être enregistré **à l'identique** sur le client SSO |
...
| Variable | Rôle |
|---|---|
| `OAUTH_ISSUER` | Par exemple `https://open.gerege.mn`. Le fournisseur n'est activé **que** si elle est définie |
