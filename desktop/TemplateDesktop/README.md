...
### Бүрхүүлийн дотоод дэлгэцүүд

Апп бараг бүхэлдээ web-ийн UI-г ачаалдаг ч **хоёр дэлгэц нь бүрхүүлийнх**:
`static/offline.html` (сервер хүрэхгүй) ба `static/server.html` (сервер солих).
Эдгээр нь `file://` хуудсууд тул web-ийн CSS-д хүрэхгүй.

**Дүрэм: `static/shell.css` нь `frontend/src/app/globals.css`-тэй lockstep-д
байна.** Токен (өнгө, радиус, фонт) ба примитив (`.card`, `.btn`, `.input`,
`.field`) бүр тэндээс хуулбарлагдсан — `shell.css`-д шинэ hex/өнгө бүү нэмэгтүн,
`globals.css` өөрчлөгдвөл энэ файлыг мөн шинэчилнэ. Шалтгаан: эдгээр дэлгэц нь
аппын **өмнө** гарч ирдэг (сүлжээ тасрах, анхны тохиргоо) тул өөр палитртай бол
хэрэглэгчид өөр программ шиг мэдрэгдэнэ. Энэ дэг журмыг
[eid-platform-mn](https://github.com/gerege-systems/eid-platform-mn)-ийн desktop
клиентүүдээс авав — тэдгээрийн macOS (`Design/Colors.swift`) ба Windows
(`Colors.xaml`) давхарга нь мөн web-ийн `globals.css`-ээс тэжээгддэг.

Хоёр зөрүү нь зориудынх: (1) загварыг `prefers-color-scheme`-ээр барина — web-ийн
`html[data-theme]`-ыг тавьдаг bootstrap script ч, хэрэглэгчийн сонголт ч
`file://` хуудсанд байхгүй; (2) фонтын стек шууд бичигдсэн — `next/font`-ийн
хувьсагчид энд хүрэхгүй.
