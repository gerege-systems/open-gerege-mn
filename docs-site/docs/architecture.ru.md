...
| **Фронтенд** | Next.js 16 (BFF) + `@gerege/ui-core` | Браузер общается только со своим origin; токены не попадают в клиентский JS |
...
Ядро **однослойное**: **`open-gerege-core`** — его напрямую используют
как государственная линия, так и линия Gerege. (До 2026-08-02 сверху
находился закрытый слой `private-gerege-core`; он убран из цепочки.)
